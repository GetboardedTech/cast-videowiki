  ...
  <div>
    ...
    <div class="container py-10">
      ...
      <div  style="margin-top: 5%;">
        <h1  class="font-extrabold text-4xl mb-4"><p>Your CAST will be live in <vue-countdown-timer
      ...
      <div class="eventDetails w-full border p-10 con-vs-card shadow-none">
        ...
        <div class="flex items-start">
          ...
          <img :src="e_cover_image" name="eventCoverPic" alt="Event Cover" class="rounded" width="50" />
...
<div  v-if="emailVerify" >
        <div class="row">
          ...
          v-model="email"
        />    <span class="text-danger text-sm mt-1" v-show="email_error">Email  field is required</span>
        </div>
        <vs-button
          class="btncentre"
          @click="sendOtp"
        ...
        <div  v-else>
          <div class="row">
            ...
            v-on:change="valueCheck()"
            v-model="otp"
          />    <span class="text-danger text-sm mt-1" v-show="otp_error">OTP  field is required</span>
          </div>
          <vs-button
            class="btncentre"
            @click="verifyOtp"
    ...
    methods: {
      fetchEventDetails(eventId){
        ...
        then(res => {
          console.log('evaent data',res);
            ...
            this.stream_url=Constants.streamUrl+this.$route.params.eventId+".m3u8";
      ...
      sendOtp(){
        if(this.email.length>0){
          axios
            .get(
              Constants.apiCastUrl+'/api/event/send/otp/?email='+this.email+'&cast_id='+this.$route.params.eventId).
          then(res => {
      ...
      verifyOtp(){
        if(this.email.length>0) {
          axios
            .get(
              Constants.apiCastUrl + '/api/event/validate/otp/?email=' + this.email + '&cast_id=' + this.$route.params.eventId + '&otp=' + this.otp).then(res => {
            console.log(res.data);
            this.$vs.loading()
            setTimeout( ()=> {
